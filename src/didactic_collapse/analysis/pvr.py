...
def _collect_pvr_decisions(run_dir: Path) -> pd.DataFrame:
    parts: list[pd.DataFrame] = []
    for path in sorted(run_dir.glob("*/*/gen_*/pvr_decisions.parquet")):
...
def _collect_pvr_reports(run_dir: Path) -> pd.DataFrame:
    rows: list[dict[str, object]] = []
    for path in sorted(run_dir.glob("*/*/gen_*/pvr_report.json")):
...
def _build_keep_repair_reject_quality(decisions_df: pd.DataFrame) -> pd.DataFrame:
    ...
    rows: list[dict[str, object]] = []
    for (branch, generation), grp in decisions_df.groupby(["branch", "generation"], as_index=False):
        kept = grp[grp["action"] == "keep_original"]
        repaired = grp[grp["action"] == "repair_explanation"]
        rejected = grp[grp["action"] == "reject"]
        rows.append(
            {
                "branch": str(branch),
                "generation": int(generation),
                ...
                "kept_pedagogical_mean": float(kept["overall_pedagogical_score"].mean()) if not kept.empty else None,
                ...
                "repaired_silent_error_rate": float(repaired["is_silent_error"].mean()) if not repaired.empty else None,
                "rejected_silent_error_rate": float(rejected["is_silent_error"].mean())
                if not rejected.empty
                else None,
                "keep_rate": float((grp["action"] == "keep_original").mean()),
                "repair_rate": float((grp["action"] == "repair_explanation").mean()),
                "reject_rate": float((grp["action"] == "reject").mean()),
            }
        )
    return pd.DataFrame(rows).sort_values(["branch", "generation"]).reset_index(drop=True)
...
def export_pvr_analysis(*, run_dir: Path, out_dir: Path) -> PVRAnalysisArtifacts:
    ...
    decisions = _collect_pvr_decisions(run_dir)
    reports = _collect_pvr_reports(run_dir)
    ...
    run_level_csv = out_dir / "pvr_run_level.csv"
    run_level_parquet = out_dir / "pvr_run_level.parquet"
    generation_deltas_csv = out_dir / "pvr_generation_deltas.csv"
    generation_deltas_parquet = out_dir / "pvr_generation_deltas.parquet"
    branch_deltas_csv = out_dir / "pvr_branch_deltas.csv"
    branch_deltas_parquet = out_dir / "pvr_branch_deltas.parquet"
    decision_reasons_csv = out_dir / "pvr_decision_reasons.csv"
    decision_reasons_parquet = out_dir / "pvr_decision_reasons.parquet"
    repair_success_summary_csv = out_dir / "pvr_repair_success_summary.csv"
    repair_success_summary_parquet = out_dir / "pvr_repair_success_summary.parquet"
    keep_repair_reject_quality_csv = out_dir / "pvr_keep_repair_reject_quality.csv"
    keep_repair_reject_quality_parquet = out_dir / "pvr_keep_repair_reject_quality.parquet"
    stress_summary_csv = out_dir / "pvr_stress_summary.csv"
    stress_summary_parquet = out_dir / "pvr_stress_summary.parquet"
    ...
    accuracy_plot = figures_dir / "pvr_accuracy_by_branch_generation.png"
    pedagogical_plot = figures_dir / "pvr_pedagogical_by_branch_generation.png"
    silent_error_plot = figures_dir / "pvr_silent_error_by_branch_generation.png"
    keep_rate_plot = figures_dir / "pvr_keep_rate_by_generation.png"
    ...
    metadata_json = out_dir / "pvr_metadata.json"
    metadata = {
        "run_dir": str(run_dir),
        "evaluation_mode": "inference_recycling_only",
        "notes": "PVR analysis over one run; not full retraining.",
    }
    metadata_json.write_text(json.dumps(metadata, ensure_ascii=False, indent=2), encoding="utf-8")

    return PVRAnalysisArtifacts(
