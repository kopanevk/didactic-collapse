...
from didactic_collapse.analysis.pvf_confirmatory import (
    PVFConfirmatoryArtifacts,
    export_pvf_confirmatory_analysis,
)
...
class PVFConfirmatorySummary:
    seeds: list[int]
    sample_size: int
    run_dirs: list[Path]
    analysis_dir: Path
    artifacts: PVFConfirmatoryArtifacts
...
def run_pvf_confirmatory_series(
    *,
    cfg: AppConfig,
    sample_size: int,
    seeds: Sequence[int],
    analysis_dir: Path | None = None,
) -> PVFConfirmatorySummary:
    if sample_size <= 0:
    ...
    artifacts = export_pvf_confirmatory_analysis(run_dirs=run_dirs, out_dir=out_dir)
    return PVFConfirmatorySummary(
