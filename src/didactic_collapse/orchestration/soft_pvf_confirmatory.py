...
from didactic_collapse.analysis.soft_pvf_confirmatory import (
...
class SoftPVFConfirmatorySummary:
    seeds: list[int]
    sample_size: int
    run_dirs: list[Path]
    analysis_dir: Path
    artifacts: SoftPVFConfirmatoryArtifacts
...
def run_soft_pvf_confirmatory_series(
    *,
    cfg: AppConfig,
    sample_size: int,
    seeds: Sequence[int],
    analysis_dir: Path | None = None,
) -> SoftPVFConfirmatorySummary:
    if sample_size <= 0:
    ...
    artifacts = export_soft_pvf_confirmatory_analysis(run_dirs=run_dirs, out_dir=out_dir)
    return SoftPVFConfirmatorySummary(
...
__all__ = [
    "SoftPVFConfirmatorySummary",
    "parse_seed_list",
    "run_soft_pvf_confirmatory_series",
]
