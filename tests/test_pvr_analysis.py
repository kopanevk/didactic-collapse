...
def _make_fake_run(root: Path) -> Path:
    run_dir = root / "pvr_fake_run"
    run_dir.mkdir(parents=True, exist_ok=True)
    (run_dir / "tables").mkdir(parents=True, exist_ok=True)
    (run_dir / "run_config.snapshot.json").write_text(
        json.dumps({"config": {"project": {"seed": 321}}}, ensure_ascii=False, indent=2),
        encoding="utf-8",
    )
    ...
    for branch in ["pure_recycling", "pvr_repair_medium"]:
    ...
    pvr_ctx = model_dir / "pvr_repair_medium" / "gen_0"
    decisions = pd.DataFrame(
        [
            ...
            },
            {
                ...
                "decision_reason": "pedagogical_repair_needed",
    ...
    decisions.to_parquet(pvr_ctx / "pvr_decisions.parquet", index=False)
    report = {
        "model_name": "qwen2.5:0.5b",
        "branch": "pvr_repair_medium",
        "generation": 0,
        ...
        "repair_count": 1,
        "reject_count": 1,
        "keep_rate": 0.3333333333,
        "repair_rate": 0.3333333333,
        "reject_rate": 0.3333333333,
        "repair_attempted_count": 1,
        "repair_success_count": 1,
        "repair_success_rate": 1.0,
        "decision_reason_counts": {"high_quality": 1, "pedagogical_repair_needed": 1, "accuracy_incorrect": 1},
    }
    (pvr_ctx / "pvr_report.json").write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    return run_dir


def test_export_pvr_analysis_writes_expected_outputs() -> None:
    base = Path("outputs/.tmp") / f"pvr_analysis_{uuid.uuid4().hex}"
    base.mkdir(parents=True, exist_ok=True)
    try:
        run_dir = _make_fake_run(base)
        out_dir = base / "analysis" / "tables"
        artifacts = export_pvr_analysis(run_dir=run_dir, out_dir=out_dir)
        ...
        assert {"branch", "generation", "keep_rate", "repair_rate", "reject_rate"}.issubset(stress.columns)
    finally:
